...
sub init_from_file {
    ...
    $new_obj->cached_property_init( "taggings", ["id"], ["feed_id", "name"] );
...
sub subscriptions {
    ...
    $self->{subscriptions}->set( $self->get_json("subscriptions.json") );
...
sub taggings {
    my ( $self, $force ) = @_;
    return $self->{taggings}->set( $self->get_json("taggings.json") );
}    ##  taggings
